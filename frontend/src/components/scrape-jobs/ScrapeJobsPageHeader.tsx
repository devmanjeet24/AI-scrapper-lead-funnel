...
import {
  ScrapeJobsFiltersPanel,
  type ScrapeJobFilters,
} from '@/components/scrape-jobs/ScrapeJobsFiltersPanel'
import { cn } from '@/lib/utils'

interface ScrapeJobsPageHeaderProps {
  totalCount: number
  displayedCount: number
  searchQuery: string
  filters: ScrapeJobFilters
  onSearchChange: (value: string) => void
  onFiltersChange: (filters: ScrapeJobFilters) => void
  onCreateJob: () => void
}

export function ScrapeJobsPageHeader({
  totalCount,
  displayedCount,
  searchQuery,
  filters,
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            {displayedCount === totalCount
              ...
              : `Showing ${displayedCount} of ${totalCount} jobs`}
          </p>
        </div>
